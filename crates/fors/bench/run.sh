...
FORMAT="${FORMAT:-both}"
PARAM_SETS="${PARAM_SETS:-SLH-DSA-SHA2-128s,SLH-DSA-SHA2-128f,SLH-DSA-SHA2-192s,SLH-DSA-SHA2-192f,SLH-DSA-SHA2-256s,SLH-DSA-SHA2-256f}"
MSG_SIZES="${MSG_SIZES:-32,256,1024,4096}"
ITERATIONS="${ITERATIONS:-100}"
WARMUP_RUNS="${WARMUP_RUNS:-3}"
RUNS="${RUNS:-5}"
OPERATIONS="${OPERATIONS:-keygen,sign,verify}"
ALG_NAME="${ALG_NAME:-FORS (SLH-DSA)}"
LIB_NAME="${LIB_NAME:-fors}"
LIB_COMMIT="${LIB_COMMIT:-unknown}"
RNG_SOURCE="${RNG_SOURCE:-unknown}"
DETERMINISTIC_RNG="${DETERMINISTIC_RNG:-0}"
RNG_SEED="${RNG_SEED:-}"
PREHASH="${PREHASH:-0}"
CANARY_CHECK="${CANARY_CHECK:-0}"
STACK_CHECK="${STACK_CHECK:-0}"
...
Usage: run.sh [--bench-cmd "cmd"] [--out-dir path] [--format json|csv|both]
             ...
             [--deterministic-rng] [--rng-seed value] [--prehash]
             [--canary-check] [--stack-check] [--hash-only] [--code-size]
...
Optional env vars:
  ...
  DETERMINISTIC_RNG=1, RNG_SEED=..., PREHASH=1, CANARY_CHECK=1,
  STACK_CHECK=1, HASH_ONLY=1, CODE_SIZE=1, PRINT_SUMMARY=1.
...
if $csv_enabled; then
  ...
  printf "%s\n" \
    "run_id,timestamp_utc,host,os_kernel,cpu_model,cpu_microcode,ram_bytes,compiler_name,compiler_version,compiler_flags,library_name,library_commit,algorithm_name,turbo_scaling,rng_source,deterministic_rng,rng_seed,prehash,canary_check,stack_check,hash_only,code_size,workspace_commit,bench_command,iterations,warmup_runs,measurement_runs,operation,param_set,message_size,run_index,total_ns,avg_ns,throughput_ops_per_s" \
    > "$csv_file"
fi
...
append_measurement_csv() {
  ...
  printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    ...
    "$(csv_escape "$DETERMINISTIC_RNG")" \
    "$(csv_escape "$RNG_SEED")" \
    "$(csv_escape "$PREHASH")" \
    "$(csv_escape "$CANARY_CHECK")" \
    "$(csv_escape "$STACK_CHECK")" \
    "$(csv_escape "$HASH_ONLY")" \
    "$(csv_escape "$CODE_SIZE")" \
  ...
  cat > "$json_file" <<EOF
{
  ...
  "metadata": {
    ...
    "environment": {
      ...
      "deterministic_rng": "$(json_escape "$DETERMINISTIC_RNG")",
      "rng_seed": "$(json_escape "$RNG_SEED")",
      "prehash": "$(json_escape "$PREHASH")",
      "canary_check": "$(json_escape "$CANARY_CHECK")",
      "stack_check": "$(json_escape "$STACK_CHECK")",
      "hash_only": "$(json_escape "$HASH_ONLY")",
      "code_size": "$(json_escape "$CODE_SIZE")",
...
PY
      ...
      awk -F',' 'NR==1 {next} {key=$28 FS $29 FS $30; avg[key]+=$33; thr[key]+=$34; cnt[key]++}
      END {
        printf "\nSummary (avg across runs)\n";
