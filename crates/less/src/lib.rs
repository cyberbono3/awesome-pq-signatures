...
pq_bench::declare_ffi_signed_message_backend!(
    native_type = NativeLess,
    init_rng = init_rng,
    seed_bytes = LESS_SEED_BYTES,
    seed_length_error = "less seed length fits in u32",
    init_rng_fn = less_rs_init_rng,
    public_key_bytes_fn = less_rs_public_key_bytes,
    secret_key_bytes_fn = less_rs_secret_key_bytes,
    signature_bytes_fn = less_rs_signature_bytes
);
