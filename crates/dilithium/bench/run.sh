...
FORMAT="${FORMAT:-both}"
PARAM_SETS="${PARAM_SETS:-ML-DSA-44,ML-DSA-65,ML-DSA-87}"
MSG_SIZES="${MSG_SIZES:-32,256,1024,4096}"
ITERATIONS="${ITERATIONS:-100}"
WARMUP_RUNS="${WARMUP_RUNS:-3}"
RUNS="${RUNS:-5}"
OPERATIONS="${OPERATIONS:-keygen,sign,verify}"
ALG_NAME="${ALG_NAME:-Dilithium (ML-DSA)}"
LIB_NAME="${LIB_NAME:-ml-dsa}"
...
Example:
  BENCH_CMD='cargo run --release --bin dilithium_bench --' \
