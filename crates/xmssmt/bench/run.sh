...
FORMAT="${FORMAT:-both}"
PARAM_SETS="${PARAM_SETS:-XMSSMT-SHA2_20/2_256,XMSSMT-SHA2_40/2_256}"
MSG_SIZES="${MSG_SIZES:-32,256,1024,4096}"
ITERATIONS="${ITERATIONS:-100}"
WARMUP_RUNS="${WARMUP_RUNS:-3}"
RUNS="${RUNS:-5}"
OPERATIONS="${OPERATIONS:-keygen,sign,verify}"
ALG_NAME="${ALG_NAME:-XMSSMT}"
LIB_NAME="${LIB_NAME:-xmss-rs}"
...
ALLOW_LIB_OVERRIDE="${ALLOW_LIB_OVERRIDE:-0}"
...
Optional env vars:
  LIB_NAME (default xmss-rs), LIB_COMMIT, ALG_NAME, RNG_SOURCE, TURBO_STATE,
  COMPILER_NAME, COMPILER_VERSION, COMPILER_FLAGS, RUN_ID,
  PRINT_SUMMARY=1, ALLOW_LIB_OVERRIDE=1.

Example:
  BENCH_CMD='cargo run --release --bin xmssmt_bench --' \
...
if [[ "$ALLOW_LIB_OVERRIDE" != "1" && "$LIB_NAME" != "xmss-rs" ]]; then
  echo "LIB_NAME must be xmss-rs for this harness (got '$LIB_NAME')." >&2
  echo "Set ALLOW_LIB_OVERRIDE=1 to override." >&2
  exit 1
fi
