        ...
        .bench_values(|mut secret_key| {
            black_box(
                scheme
                    ...
                    .expect("LMS sign benchmark should succeed"),
            );
        });
}
...
fn print_sizes() {
    println!("LMS sizes:");
    for param_set in LMS_PARAM_SETS {
        let scheme = LmsScheme::new(param_set);
        ...
        println!(
            ...
            signed_message_size(32, sizes.signature_bytes)
        );
        println!(
            "    Estimated signatures per key: {}",
            scheme.max_signatures_per_key()
        );
    }
}
