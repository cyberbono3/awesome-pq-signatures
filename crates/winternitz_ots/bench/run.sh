...
FORMAT="${FORMAT:-both}"
PARAM_SETS="${PARAM_SETS:-w4-n32-sha256,w16-n32-sha256}"
MSG_SIZES="${MSG_SIZES:-32,256,1024,4096}"
ITERATIONS="${ITERATIONS:-100}"
WARMUP_RUNS="${WARMUP_RUNS:-3}"
RUNS="${RUNS:-5}"
OPERATIONS="${OPERATIONS:-keygen,sign,verify}"
ALG_NAME="${ALG_NAME:-Winternitz OTS (W-OTS)}"
LIB_NAME="${LIB_NAME:-winternitz-ots}"
...
Example:
  BENCH_CMD='cargo run --release --bin winternitz_ots_bench --' \
