...
In practice, this means the current benchmark set is aligned where the workspace can select a like-for-like security target, but it is not claiming that every scheme row exposes an identical knob or an identical security model.
